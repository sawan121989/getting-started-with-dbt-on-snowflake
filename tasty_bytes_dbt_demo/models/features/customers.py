...
def model(dbt, session):
    df_cust_trx_fraud = session.table('SNOWFLAKE_LEARNING_DB.PUBLIC.CUSTOMER_TRANSACTIONS_FRAUD')
    ...
    distinct_customers = session.sql('SELECT DISTINCT customer_id FROM SNOWFLAKE_LEARNING_DB.PUBLIC.CUSTOMER_TRANSACTIONS_FRAUD ORDER BY customer_id')
    ...
    win_90d_cust = cust_date.rowsBetween(-90, -1)
    win_365d_cust = cust_date.rowsBetween(-365, -1)
