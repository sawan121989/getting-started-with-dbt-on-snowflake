{% test validate_src_tgt_aggregation_xxxxx(
    model,
    test_id,
    start_dt=None,
    end_dt=None
    ...
    {% else %}
        ...
                    {% for i in range(1, src_groups|length + 1) %}
                    ...
                    {% endfor %},
                    {% for i in range(1, src_measures|length + 1) %}
                        s.group_col_{{ i }} as src_{{ m }},
                        t.group_col_{{ i }} as tgt_{{ m }},
                        abs(coalesce(s.group_col_{{ i }},0) - coalesce(t.group_col_{{ i }},0)) as diff_{{ m }},
                        case 
                            when abs(coalesce(s.group_col_{{ i }},0) - coalesce(t.group_col_{{ i }},0))
                                 <= coalesce({{ threshold | default(0) }},0) 
                                    * nullif(abs(coalesce(t.group_col_{{ i }},0)),0)
                            then 0 else 1 end as fail_flag_{{ m }}
